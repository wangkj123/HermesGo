...
_LOCAL_CUSTOM_PROVIDER_ALIASES = {
    "ollama",
    "lmstudio",
    "lm-studio",
    "lm_studio",
...
class AIAgent:
    ...
    def __init__(
        ...
        if provider_name in _LOCAL_CUSTOM_PROVIDER_ALIASES:
        ...
        if self.api_mode == "anthropic_messages":
            ...
            client_kwargs = None
            runtime_requested_provider = provider_name
            if base_url and not runtime_requested_provider:
                runtime_requested_provider = "custom"
            ...
            if runtime and runtime.get("base_url"):
                ...
                if (
                    api_mode is None
                    and isinstance(resolved_api_mode, str)
                    and resolved_api_mode in {
                        "chat_completions",
                        "codex_responses",
                        "anthropic_messages",
                        "bedrock_converse",
                ...
                elif "api.githubcopilot.com" in resolved_base.lower():
                    from hermes_cli.models import copilot_default_headers

                    client_kwargs["default_headers"] = copilot_default_headers()
                elif "api.kimi.com" in resolved_base.lower():
                    client_kwargs["default_headers"] = {
                        "User-Agent": "KimiCLI/1.30.0",
                    }
                elif "portal.qwen.ai" in resolved_base.lower():
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      consumed by another process. To fix:", force=True)
                                self._vprint(f"{self.log_prefix}      1. Disconnect and sign in again inside HermesGo.", force=True)
                                self._vprint(f"{self.log_prefix}      2. Then retry the request.", force=True)
                            else:
