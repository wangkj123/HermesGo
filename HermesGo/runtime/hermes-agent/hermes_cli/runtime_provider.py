...
def _get_model_config() -> Dict[str, Any]:
    ...
    if isinstance(model_cfg, dict):
        ...
        if provider in _LOCAL_CUSTOM_PROVIDER_ALIASES:
            cfg["provider"] = "custom"
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    if requested and requested.strip():
        ...
        if requested_value in _LOCAL_CUSTOM_PROVIDER_ALIASES:
            return "custom"
    ...
    if isinstance(cfg_provider, str) and cfg_provider.strip():
        requested_value = cfg_provider.strip().lower()
        if requested_value in _LOCAL_CUSTOM_PROVIDER_ALIASES:
            return "custom"
        return requested_value
    ...
    if env_provider:
        if env_provider in _LOCAL_CUSTOM_PROVIDER_ALIASES:
            return "custom"
        return env_provider
