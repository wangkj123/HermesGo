    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            if needs_init:
                ...
                #
                # Legacy DBs created before columns like ``session_id`` were in
                # CREATE TABLE must be migrated *before* SCHEMA_SQL's CREATE
                # INDEX statements — otherwise executescript fails with "no such
                # column" and dashboard Kanban returns HTTP 500.
                if path.exists() and path.stat().st_size > 0 and _table_exists(conn, "tasks"):
