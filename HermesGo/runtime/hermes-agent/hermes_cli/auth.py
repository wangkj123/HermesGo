...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    with _auth_store_lock():
        ...
        if not cleared and not local_cleared:
            return False
...
# =============================================================================
# OpenAI Codex auth — tokens stored only in Hermes auth.json
# 
# HermesGo keeps its own Codex OAuth session under the app directory.
# It must not read or write ~/.codex/auth.json.
# =============================================================================

def _get_local_codex_home() -> Path:
    """Return HermesGo's private Codex CLI home directory."""
    path = get_hermes_home() / "codex"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _get_local_codex_profile_home() -> Path:
    """Return the HOME/USERPROFILE root used by HermesGo's isolated Codex login."""
    path = get_hermes_home() / "codex-home"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _get_local_codex_auth_path() -> Path:
    return _get_local_codex_home() / "auth.json"


def _display_local_codex_auth_path() -> str:
...
def _clear_local_codex_auth_files() -> bool:
    ...
    for path in (*_codex_auth_file_paths(), *_codex_auth_lock_paths()):
        try:
            ...
            logger.debug("Failed to remove local Codex auth file %s: %s", path, exc)
    return removed
...
def _codex_browser_oauth_login(
    *,
    open_browser: bool = True,
    preferred_port: Optional[int] = None,
) -> Dict[str, Any]:
    """Run the OpenAI Codex browser OAuth flow and return credentials dict.

    ``preferred_port`` controls the local callback listener port.  When set to
    0 the server binds an ephemeral loopback port, which is useful for UIs that
    want to avoid a fixed 1455 listener.
    """
    verifier, challenge = _generate_pkce_pair()
    state = base64.urlsafe_b64encode(os.urandom(24)).rstrip(b"=").decode("ascii")
    port = _get_codex_oauth_redirect_port() if preferred_port is None else preferred_port
    server, redirect_uri, result, done = _create_codex_oauth_callback_server(preferred_port=port)
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, auth_mode: str = "chatgpt") -> None:
    ...
    with _auth_store_lock():
        ...
        env_base_url = os.getenv("HERMES_CODEX_BASE_URL", "").strip().rstrip("/")
        existing_base_url = str(state.get("base_url") or "").strip().rstrip("/")
...
def _get_configured_codex_base_url() -> str:
    """Return the configured Codex base_url from config.yaml when applicable."""
    try:
    ...
    *,
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                "Codex refresh token was already consumed by another process. "
                "Re-authenticate inside HermesGo to create a fresh app-local session."
            )
            relogin_required = True
        raise AuthError(
            message,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    env_base_url = os.getenv("HERMES_CODEX_BASE_URL", "").strip().rstrip("/")
    stored_base_url = str(data.get("base_url") or "").strip().rstrip("/")
    config_base_url = _get_configured_codex_base_url()
    base_url = env_base_url or stored_base_url or config_base_url or DEFAULT_CODEX_BASE_URL
            ...
            if entry is not None:
                ...
                if api_key and not _codex_access_token_is_expiring(api_key, 0):
                    return {
                        ...
                        "auth_mode": str(getattr(entry, "auth_mode", None) or "chatgpt"),
...
def _update_config_for_provider(
    ...
    existing_base_url = str(model_cfg.get("base_url") or "").strip().rstrip("/")
    resolved_base_url = inference_base_url.rstrip("/") if inference_base_url and inference_base_url.strip() else ""
    if (
        provider_id == "openai-codex"
        and previous_provider == "openai-codex"
        and existing_base_url
        and (not resolved_base_url or resolved_base_url == DEFAULT_CODEX_BASE_URL)
    ):
        resolved_base_url = existing_base_url
...
def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via HermesGo-local browser login only."""
    del args
    del pconfig
    ...
    try:
        ...
        if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
            ...
            try:
                reuse = input("Use existing credentials? [Y/n/switch]: ").strip().lower()
    ...
    # Run a fresh browser login through an isolated Codex CLI home under HermesGo.
    print()
    print("Signing in to OpenAI Codex...")
    print("(HermesGo stores this session only in its own local directory)")
    print()

    creds = _codex_cli_browser_login(open_browser=not getattr(args, "no_browser", False))
