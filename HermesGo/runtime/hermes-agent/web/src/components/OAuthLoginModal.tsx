...
export function OAuthLoginModal({
  ...
  const beginLogin = useCallback(async () => {
    ...
    try {
      const resp = await api.startOAuthLogin(provider.id, { switchAccount });
  ...
  useEffect(() => {
    ...
    const tick = window.setInterval(() => {
      ...
      setSecondsLeft((current) => {
        ...
        return current !== null && current > 0 ? current - 1 : 0;
      });
    }, 1000);
  ...
  useEffect(() => {
    if (!start || phase !== "polling") {
      return;
    }
    if (start.flow !== "device_code" && start.flow !== "browser") {
      return;
    }
    ...
    pollTimer.current = window.setInterval(async () => {
      try {
        const resp = await api.pollOAuthSession(provider.id, sessionId);
        if (!isMounted.current) {
          return;
        }
  ...
  const fmtTime = (seconds: number | null) => {
    if (seconds === null) {
      return "";
    }
      ...
      <div className="relative w-full max-w-md border border-border bg-card shadow-2xl">
        ...
        <div className="p-6 flex flex-col gap-4">
            ...
            <>
              ...
              <div className="flex items-center justify-between gap-2 border border-border bg-secondary/30 p-4">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    void handleCopyUserCode(start.user_code);
                  }}
                  className="text-xs"
                >
          ...
          {start?.flow === "browser" && phase === "polling" && (
            <>
              <div className="border border-border bg-secondary/20 p-4 text-sm text-muted-foreground space-y-2">
