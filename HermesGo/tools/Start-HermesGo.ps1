...
$dashboardHost = if ($env:HERMESGO_DASHBOARD_HOST) { $env:HERMESGO_DASHBOARD_HOST } else { "127.0.0.1" }
$dashboardPort = if ($env:HERMESGO_DASHBOARD_PORT) { [int]$env:HERMESGO_DASHBOARD_PORT } else { 9119 }
$dashboardUrl = if ($env:HERMESGO_DASHBOARD_URL) { $env:HERMESGO_DASHBOARD_URL } else { "http://127.0.0.1:$dashboardPort/" }
$pythonExe = if ($env:HERMESGO_PYTHON_EXE) { $env:HERMESGO_PYTHON_EXE } else { Join-Path $appRoot "runtime\python311\python.exe" }
$runtimeDir = Join-Path $appRoot "runtime\hermes-agent"
$hermesHomePath = Join-Path $appRoot "home"
...
function Ensure-LocalOllamaReady {
    ...
    if (Test-ListeningPort -Port $ollamaConfig.Port) {
        ...
        if ($ownerPath -and $ownerPath.ToLowerInvariant() -eq $ollamaExe.ToLowerInvariant()) {
            ...
            Stop-PortOwner -Port $ollamaConfig.Port
        }
    }
    ...
    foreach ($pid in $pids) {
        if ($pid -and $pid -ne $PID) {
            try {
                ...
                Write-LauncherLine "Stopped existing process on port ${Port}: PID $pid"
            }
            catch {
                Write-LauncherLine "Failed to stop process on port ${Port}: PID $pid"
            }
        }
    }
}
...
function Start-DashboardProcess {
    ...
    Stop-PortOwner -Port $dashboardPort
    Remove-Item -LiteralPath $dashboardOutLog, $dashboardErrLog -Force -ErrorAction SilentlyContinue

    $arguments = @("-m", "hermes_cli.main", "dashboard", "--host", $dashboardHost, "--port", "$dashboardPort", "--no-open")
...
function Start-ChatWindow {
    ...
    $command = 'set HERMES_HOME=' + $hermesHomePath +
    ...
    $process = Start-Process -FilePath "cmd.exe" -ArgumentList "/k", $command -WorkingDirectory $appRoot -PassThru
...
try {
    ...
    Write-LauncherLine "App root: $appRoot"
    Write-LauncherLine "Runtime dir: $runtimeDir"
    Write-LauncherLine "Home dir: $hermesHomePath"
    ...
    foreach ($requiredPath in @($pythonExe, $runtimeDir, $hermesHomePath)) {
